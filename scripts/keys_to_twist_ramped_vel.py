...
def ramped_vel(v_prev, v_target, t_prev, t_now, ramp_rate):
    #compute max vel step
    step = ramp_rate * (t_now - t_prev).to_sec()
    sign = 1.0 if(v_target > v_prev) else -1.0
    error = math.fabs(v_target - v_prev)
    if (error < step): # we can reach the target vel in the next step
        return v_target
    else:
...
def ramped_twist(prev, target, t_prev, t_now, ramps):
    tw = Twist()
    tw.angular.z = ramped_vel(prev.angular.z, target.angular.z, 
                                t_prev, t_now, ramps[0])
    tw.linear.x = ramped_vel(prev.linear.x, target.linear.x, 
                                    t_prev, t_now, ramps[1])
    return tw
...
def keys_cb(msg):
    ...
    if (len(msg.data) == 0 or not key_mapping.get(msg.data[0])):
        return # unknown key 
